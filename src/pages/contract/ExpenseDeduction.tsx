...
export default function ExpenseDeduction() {
  ...
  const attach = useContractFormAttachments('expense-deductions');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const deductionCode = 'EX-DED-' + Date.now();
    const submitData = { ...form, deduction_code: form.deduction_code || deductionCode, attachment_url };
    
    if (form.id) {
      const { error } = await supabase.from('expense_deductions').update(submitData).eq('id', form.id);
      if (error) { alert('保存失败: ' + error.message); return; }
      alert('更新成功');
    } else {
      const { data, error } = await supabase.from('expense_deductions').insert(submitData).select().single();
      if (error) { alert('保存失败: ' + error.message); return; }

      const approvalSubmitted = await tryCreateApproval('expense_deduction', data.id, `扣款 ${data.deduction_code}`);
  ...
  async function handleDelete(id: string) {
    ...
    await supabase.from('expense_deductions').delete().eq('id', id);
    refreshAll();
  }

  const filteredData = filterRowsBySearch(listData, search, ['deduction_code', 'deduction_reason']);

  const getContractName = (contractId: string) => contractNameById(contracts, contractId);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-bold text-gray-800">支出合同扣款登记</h3>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-gray-700">扣款金额</th>
              ...
              <tbody>
                {filteredData.map((item: any) => (
                  <tr key={item.id} className="border-b border-gray-200/50 hover:bg-gray-50/30">
                    ...
                    <td className="px-4 py-3 text-red-400">{item.deduction_amount ? item.deduction_amount.toFixed(2) : '-'}</td>
                    <td className="px-4 py-3 text-gray-700">{item.deduction_date || '-'}</td>
                    <td className="px-4 py-3 text-center"><ApprovalStatusBadge sourceType="expense_deduction" sourceId={item.id} sourceName={`扣款 ${item.deduction_code}`} /></td>
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => { setForm(item); attach.loadFromField(item.attachment_url); setShowModal(true); }} className="p-2 text-blue-400 hover:bg-blue-500/20 rounded-lg mr-2"><FaEdit /></button>
                      <button onClick={() => handleDelete(item.id)} className="p-2 text-red-400 hover:bg-red-500/20 rounded-lg"><FaTimes /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white rounded-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div><label className="block text-sm text-gray-500 mb-2">扣款事由 *</label><textarea required value={form.deduction_reason || ''} onChange={e => setForm({ ...form, deduction_reason: e.target.value })} className="w-full px-4 py-2 bg-gray-50 border border-slate-600 rounded-lg text-gray-800" rows={2} /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-sm text-gray-500 mb-2">扣款金额 *</label><input type="number" step="0.01" required value={form.deduction_amount || ''} onChange={e => setForm({ ...form, deduction_amount: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-2 bg-gray-50 border border-slate-600 rounded-lg text-gray-800" /></div>
                  <div><label className="block text-sm text-gray-500 mb-2">扣款日期 *</label><input type="date" required value={form.deduction_date || ''} onChange={e => setForm({ ...form, deduction_date: e.target.value })} className="w-full px-4 py-2 bg-gray-50 border border-slate-600 rounded-lg text-gray-800" /></div>
                </div>
