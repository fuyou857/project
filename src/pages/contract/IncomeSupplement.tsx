...
export default function IncomeSupplement() {
  const { contracts, listData, loading, refreshAll } = useContractSideList(
    tables.contracts,
    tables.supplements,
    { listLimit: PAGE_SIZE, contractSelect: 'id, contract_name, contract_amount', listContractFk: 'main_contract_id' },
  );
  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState<any>({});
  const [search, setSearch] = useState('');
  const attach = useContractFormAttachments('income-supplements');

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const missing: string[] = [];
    if (!form.main_contract_id) missing.push('关联主合同');
    if (!form.supplement_amount && form.supplement_amount !== 0) missing.push('补充金额');
    ...
    if (form.id) {
      const { error } = await supabase.from('income_supplements').update(payload).eq('id', form.id);
      if (error) { alert('保存失败: ' + error.message); return; }
      await applySupplementAmountChange('income', {
        isCreate: false,
      ...
      const code = 'SP' + new Date().toISOString().slice(0, 10).replace(/-/g, '') + String(Math.floor(Math.random() * 10000)).padStart(4, '0');
      const submitData = { ...payload, supplement_code: code };
      const { data, error } = await supabase.from('income_supplements').insert(submitData).select().single();
      if (error) { alert('保存失败: ' + error.message); return; }

      const approvalSubmitted = await tryCreateApproval('income_supplement', data.id, data.supplement_name || '补充协议');
      await applySupplementAmountChange('income', {
        isCreate: true,
        mainContractId: form.main_contract_id,
        supplementAmount: form.supplement_amount,
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-bold text-gray-800">补充协议签约管理</h3>
              ...
              <tbody>
                {filteredData.map((item: any) => (
                  <tr key={item.id} className="border-b border-gray-200/50 hover:bg-gray-50/30">
                    ...
                    <td className="px-4 py-3 text-center"><ApprovalStatusBadge sourceType="income_supplement" sourceId={item.id} sourceName={item.supplement_name || '补充协议'} /></td>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-sm text-gray-500 mb-2">补充金额(万元) *</label>
                    <input type="number" step="0.01" required value={form.supplement_amount || ''} onChange={e => setForm({ ...form, supplement_amount: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-2 bg-gray-50 border border-slate-600 rounded-lg text-gray-800" />
                  </div>
