...
export default function IncomeVariation() {
  ...
  const attach = useContractFormAttachments('income-variations');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    let payload: Record<string, unknown> = { ...form, attachment_url };
    if (!payload.variation_code) {
      payload.variation_code = generateDatedRandomCode('VAR-');
    }
    payload = await omitVariationApprovalStatusIfHasWorkflow('income_variation', form.id, payload);
    
    if (form.id) {
      const { error } = await supabase.from('income_variations').update(payload).eq('id', form.id);
      if (error) { alert('保存失败: ' + error.message); return; }
      alert('更新成功');
    } else {
      const { data, error } = await supabase.from('income_variations').insert(payload).select().single();
      if (error) { alert('保存失败: ' + error.message); return; }

      const approvalSubmitted = await tryCreateApproval('income_variation', data.id, data.variation_name);
  ...
  async function handleDelete(id: string) {
    ...
    await supabase.from('income_variations').delete().eq('id', id);
    refreshAll();
  }
  ...
  const contractOptions = useMemo(
    () => contractSelectOptions(contracts, '选择主合同', 'nameCode'),
    [contracts],
  );
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-bold text-gray-800">变更签证</h3>
      ...
      <div className="bg-white rounded-xl p-6 border border-gray-200">
        <div className="flex gap-4 mb-4">
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="搜索..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-slate-600 rounded-lg text-gray-800" />
          </div>
        </div>
              ...
              <tbody>
                {filteredData.map((item: any) => (
                  <tr key={item.id} className="border-b border-gray-200/50 hover:bg-gray-50/30">
                    ...
                    <td className="px-4 py-3 text-center"><ApprovalStatusBadge sourceType="income_variation" sourceId={item.id} sourceName={item.variation_name} /></td>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="bg-white rounded-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-sm text-gray-500 mb-2">签证金额(万元) *</label><input type="number" step="0.01" required value={form.variation_amount || ''} onChange={e => setForm({ ...form, variation_amount: parseFloat(e.target.value) || 0 })} className="w-full px-4 py-2 bg-gray-50 border border-slate-600 rounded-lg text-gray-800" /></div>
