...
import { SearchableSelect, SegmentedControl, Skeleton } from '../../components/ui';
...
export default function ExpenseContractList() {
  ...
  function buildContractSummaryRows(c: {
    ...
    const partyBName = partyBs.find((p) => p.id === c.party_b_id)?.unit_name || '';
    const amountText =
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (editingId) {
      ...
      const approvalSubmitted = await tryCreateApproval('expense_contract', data.id, data.contract_name, {
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center justify-center gap-1">
                          <SubmitApprovalListAction
                            ...
                            sourceId={item.id}
                            sourceName={item.contract_name}
                            projectId={item.project_id}
                            summaryRows={buildContractSummaryRows(item)}
                            onDone={(ok) =>
                              showToast(
                                'success',
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex justify-end gap-3 pt-4"><button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setShowModal(false); }} className="px-4 py-2 bg-gray-500 text-gray-800 rounded-lg">取消</button><button type="submit" disabled={saving} className="px-4 py-2 bg-blue-600 text-gray-800 rounded-lg disabled:opacity-50">{modalSubmitText}</button></div>
              </form>
