...
import ExpenseSettlementAmountSummary from '../../components/contract/ExpenseSettlementAmountSummary';
...
interface Contract { id: string; contract_name: string; contract_code: string; contract_amount: number; }
...
export default function ExpenseSettlement() {
  ...
  const attach = useContractFormAttachments('expense-settlements');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const submitData = {
      ...form,
      attachment_url,
      settlement_code: form.settlement_code || generateDatedSequentialCode('EX-STL-', listData.length + 1),
    };
    
    if (form.id) {
      const { error } = await supabase.from('expense_settlements').update(submitData).eq('id', form.id);
      if (error) { alert('保存失败: ' + error.message); return; }
      alert('更新成功');
    } else {
      const { data, error } = await supabase.from('expense_settlements').insert(submitData).select().single();
      if (error) { alert('保存失败: ' + error.message); return; }

      const approvalSubmitted = await tryCreateApproval('expense_settlement', data.id, `结算 ${data.settlement_code}`);
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-bold text-gray-800">支出合同结算</h3>
                ...
                {filteredData.map((item: any) => (
                  <tr key={item.id} className="border-b border-gray-200/50 hover:bg-gray-50/30">
                    ...
                    <td className="px-4 py-3 text-gray-700">
                      <span className={`px-2 py-1 rounded text-xs ${item.status === '已结算' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
                        {item.status || '未结算'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center"><ApprovalStatusBadge sourceType="expense_settlement" sourceId={item.id} sourceName={`结算 ${item.settlement_code}`} /></td>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center z-50" onClick={() => setShowModal(false)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white rounded-xl p-6 w-full max-w-3xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {form.contract_id && <ExpenseSettlementAmountSummary contractId={form.contract_id} />}
